...
(function (root, window, document, factory, undefined) {
	...
	function initialise(containerSelector, options) {
		...
		function setAutoScrolling(value, type) {
			...
			if (options.autoScrolling && !options.scrollBar) {
				...
				if (element != null) {
					...
					scrollSettings.element.scrollTo(
						0,
		...
		function setResponsive(active) {
			...
			if (active) {
				if (!isResponsive) {
					...
					if (
						isFunction(
							options.afterResponsive
						)
					) {
						options.afterResponsive.call(
							container,
							active
						);
					}
		...
		function init() {
			...
			options.scrollBar =
				options.scrollBar || options.hybrid;
		...
		function delegatedEvents(e) {
			...
			if (
				target &&
				closest(target, SECTION_NAV_SEL + " a")
			) {
				...
				closest(target, SLIDES_NAV_LINK_SEL) !=
					null
			) {
		...
		function onMouseEnterOrLeave(e) {
			...
			normalSelectors.forEach(function (
				normalSelector
			) {
				if (!isInsideOneNormalScroll) {
					var isNormalScrollTarget = matches(
						target,
						normalSelector
					);
					...
					if (
						isNormalScrollTarget ||
						isNormalScrollChildFocused
					) {
						if (
							!FP.shared
		...
		function prepareDom() {
			...
			if (options.scrollOverflow) {
				scrollBarHandler = options.scrollOverflowHandler.init(
					options
				);
			}
		}
		...
		function styleSlides(section, slides, numSlides) {
			...
			var slidesContainer = document.createElement(
				"div"
			);
		...
		function addInternalSelectors() {
			addClass(
				$(options.sectionSelector, container),
				SECTION
			);
			addClass(
				$(options.slideSelector, container),
				SLIDE
			);
		}
		...
		function afterRenderActions() {
			...
			if (
				...
				fireCallback("afterLoad", {
					...
					anchorLink: section.getAttribute(
						"data-anchor"
					),
					sectionIndex: index(
						section,
		...
		function isDestinyTheStartingSection() {
			...
			return (
				!anchor.section ||
				!destinationSection ||
				(typeof destinationSection !==
					"undefined" &&
					index(destinationSection) ===
						index(
							startingSection
						))
			);
		...
		function scrollHandler() {
			...
			if (!options.autoScrolling || options.scrollBar) {
				...
				if (!hasClass(currentSection, ACTIVE)) {
					...
					var anchorLink = currentSection.getAttribute(
						"data-anchor"
					);
					var sectionIndex =
						index(
        ...
        */
		...
		function MouseWheelHandler(e) {
			...
			if (
				...
				if (canScroll) {
					var averageEnd = getAverage(
						scrollings,
						10
					);
					var averageMiddle = getAverage(
						scrollings,
						70
					);
					var isAccelerating =
		...
		function moveSlide(direction, section) {
			...
			if (destiny == null) {
				...
				var slideSiblings = siblings(
					currentSlide
				);
				if (direction === "left") {
					destiny =
						slideSiblings[
							slideSiblings.length -
								1
						]; //last
				} else {
		...
		function getDestinationPosition(element) {
			...
			var isScrollingDown =
				elementTop > previousDestTop;
			var sectionBottom =
				position -
				windowsHeight +
				elementHeight;
			var bigSectionsDestination =
				options.bigSectionsDestination;
		...
		function scrollPage(element, callback, isMovementUp) {
			...
			var v = {
				...
				anchorLink: element.getAttribute(
					"data-anchor"
				),
				sectionIndex: index(
					element,
			...
			if (v.activeSlide != null) {
				slideAnchorLink = v.activeSlide.getAttribute(
					"data-anchor"
				);
			...
			if (
				...
				((!v.isMovementUp &&
					v.yMovement == "up") || // Intending to scroll down but about to go up or
					(v.isMovementUp &&
						v.yMovement ==
							"down"))
			) {
		...
		function getEventData(eventName, v) {
			...
			if (!options.v2compatible) {
				...
				paramsPerEvent = {
					...
					onLeave: function () {
						return {
							origin: nullOrSection(
								v.activeSection
							),
							destination: nullOrSection(
								v.element
							),
							direction:
								v.direction,
		...
		function performMovement(v) {
			...
			var transitionLapse = isFastSpeed
				? 700
			...
			if (isFastSpeed) {
				...
				g_transitionLapseId = setTimeout(
					function () {
						canScroll = true;
					},
					transitionLapse
				...
				g_mediaLoadedId = setTimeout(
					function () {
						if (
							...
							scrollBarHandler.createScrollBar(
								section
							);
						}
					},
		...
		function playMedia(destiny) {
			...
			$("video, audio", panel).forEach(function (
				element
			) {
				if (
		...
		function onTab(e) {
			...
			if (activeElement) {
				if (
					...
					activeElement = preventAndFocusFirst(
						e
					);
				}
			}
		...
		function isFocusOutside(e) {
			...
			var focusDestinationIndex = e.shiftKey
				? currentFocusIndex - 1
				: currentFocusIndex + 1;
			var focusDestination =
				allFocusables[focusDestinationIndex];
		...
		function sectionBulletHandler(e) {
			...
			/*jshint validthis:true */
			var indexBullet = index(
				closest(this, SECTION_NAV_SEL + " li")
			);
		...
		function slideBulletHandler(e) {
			...
			var slides = $(
				SLIDES_WRAPPER_SEL,
				closest(this, SECTION_SEL)
			)[0];
			var destiny = $(SLIDE_SEL, slides)[
		...
		function responsive() {
			var widthLimit =
				options.responsive ||
				options.responsiveWidth; //backwards compatiblity
		...
		function destroyStructure() {
			...
			$(SECTION_SEL + ", " + SLIDE_SEL).forEach(
				function (item) {
					if (
						options.scrollOverflowHandler &&
						options.scrollOverflow
					) {
						options.scrollOverflowHandler.remove(
							item
						);
					}
					removeClass(
					...
					var previousStyles = item.getAttribute(
						"data-fp-styles"
					);
					if (previousStyles) {
						item.setAttribute(
		...
		function displayWarnings() {
			...
			var msgStyle =
				"font-size: 15px;background:yellow;";
			...
			options.anchors.forEach(function (name) {
				...
				if (idAttr.length || nameAttr.length) {
					...
					var propertyName = idAttr.length
						? "id"
						: "name";
